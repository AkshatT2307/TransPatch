...
class PatchTrainer():
  def __init__(self,config,main_logger):
      ...
      self.sidewalk_dilate = getattr(config.patch, "sidewalk_dilate", 5)                 # pixels to dilate mask
  ...
  def _softmax_entropy(self, logits, dim=1, eps=1e-8):
      ...
      return -(p * p.log()).sum(dim=dim)  # (N, H, W)
  
  def _choose_patch_topleft_from_mask(self, mask_2d, S, entropy_2d=None, topk_frac=0.2):
      """
      mask_2d: (H,W) boolean tensor where True are candidate pixels
      S: patch size (square)
      entropy_2d: (H,W) float map to bias sampling; if None, uniform within mask
      Returns (y0, x0) for patch top-left, or None if no room/candidates.
      """
      H, W = mask_2d.shape
      # remove borders where a SxS patch wouldn't fit
      valid = mask_2d.clone()
      if S > 1:
          valid[:S//2, :] = False
          valid[H - (S - S//2):, :] = False
      ...
      if entropy_2d is not None:
          # pick top-k fraction by entropy inside mask then sample
          ent = entropy_2d[idx[:,0], idx[:,1]]
          k = max(1, int(topk_frac * idx.size(0)))
          topk_vals, topk_inds = torch.topk(ent, k, largest=True)
          idx = idx[topk_inds]
  
      # uniform sample among remaining candidates
      r = torch.randint(0, idx.size(0), (1,), device=idx.device).item()
      yc, xc = idx[r].tolist()
  
      # center the patch around (yc, xc) but respect image bounds
      y0 = max(0, min(yc - S // 2, H - S))
      x0 = max(0, min(xc - S // 2, W - S))
      return y0, x0
  
  def _dilate_mask(self, mask, k=5):
      if k <= 1:
          return mask
      # simple square dilation (no external deps)
      pad = k // 2
      # use max-pooling as dilation on {0,1} mask
      m = mask.float().unsqueeze(0).unsqueeze(0)  # (1,1,H,W)
      m = F.max_pool2d(m, kernel_size=k, stride=1, padding=pad)
      return (m[0,0] > 0.5)
      
  def _paste_patch(self, image, patch, y0, x0):
      """
      image: (N,3,H,W), patch: (3,S,S) in [0,1], top-left in y0,x0
      Returns: (N,3,H,W) with patch pasted (overwrites pixels).
      """
      N, C, H, W = image.shape
      S = patch.shape[-1]
      out = image.clone()
      out[:, :, y0:y0+S, x0:x0+S] = patch[None, :, :, :]
      return out
    ...
    for ep in range(start_epoch, end_epoch):
        ...
        for i_iter, batch in enumerate(self.train_dataloader, 0):
            ...
            if self.place_on_sidewalk:
                # clean logits & preds from the unpatched image
                clean_logits = clean_output  # already computed above
                clean_pred = clean_logits.argmax(dim=1)  # (N,H,W)
            
                # build sidewalk candidate mask
                sidewalk_mask = (clean_pred == self.sidewalk_id)  # (N,H,W)
            
                # optional entropy bias (sensitive regions)
                ent = None
                if self.sidewalk_entropy_bias:
                    ent = self._softmax_entropy(clean_logits)  # (N,H,W)
            
                # optional dilation to enlarge placement options
                if self.sidewalk_dilate and self.sidewalk_dilate > 1:
                    sidewalk_mask = torch.stack(
                        [self._dilate_mask(m, k=self.sidewalk_dilate) for m in sidewalk_mask],
                        dim=0
                    )
            
                # choose a location per image and paste the same patch (EOT-transformed already)
                patched_imgs = []
                for b in range(image.size(0)):
                    ...
                    if yx is None:
                        # fallback: random placement (rare when sidewalk exists)
                        H, W = image.shape[-2:]
                ...
                patched_label = true_label.long()  # GT unchanged
            else:
                # old behavior
